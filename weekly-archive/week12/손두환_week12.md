...
* 델타기호 뜻: $E[\Delta y_0] = E[\Delta Y(0)] = E[\Delta y\mid D = 0]$ 
  - $\Delta$: 시간에 따른 변화
...
-  개입 후 기간의 실험군에서만 $Y|D=1$을 관측할 수 있고, 그 외 조건 3개(개입 후 대조군, 개입 전 대조군, 개입 전 실험군)에서는 $Y|D=0$만 관측할 수 있다.  
...
- $\tau$: 처치효과, DID추정값과 같다
  - 증명  
    $\hat{\tau}_{\text{DID}} =
    \big[ E[Y_{11}] - E[Y_{10}] \big] - \big[ E[Y_{01}] - E[Y_{00}] \big]$

    $E[Y_{11}] = \alpha_1 + \gamma_1 + \tau,\quad
    E[Y_{10}] = \alpha_1 + \gamma_0 \\
    E[Y_{01}] = \alpha_0 + \gamma_1,\quad
    E[Y_{00}] = \alpha_0 + \gamma_0$

    $\hat{\tau}_{\text{DID}} \\
    = \left( (\alpha_1 + \gamma_1 + \tau) - (\alpha_1 + \gamma_0) \right) - \left( (\alpha_0 + \gamma_1) - (\alpha_0 + \gamma_0) \right) \\ 
    = \tau
    $
...
### Block Design (블록 디자인)
* 블록디자인
  - 회귀 돌리는 모양을 그림으로 나타내면 블록같다는 말.
    - 블록디자인에서 정의한 동일시간대 비교 가능한 그룹 구조는 위 회귀식의 더미 구성 및 해석 정당성의 기반이 됨
...
- 다른방법: 블록 부트스트랩 (전체 추정과정을 부트스트랩)
  - 개체별로 블록으로 지정하고 복원추출 하여 ATT계산하고 신뢰구간 계산!
...
$$
($E[Y_{it}(0) \mid D=1]$은 구할 수 없으므로 검증 불가)  
